...
use libseccomp::{ScmpFilterContext, ScmpAction, ScmpArch, ScmpSyscall};
use log::{info, debug, warn, error};
use nix::errno::Errno;
use nix::sys::{ptrace, ptrace::Options as PtraceOptions, signal::signal, signal::Signal, signal::SigHandler};
use nix::sys::wait::{WaitStatus, WaitPidFlag, wait, waitpid};
...
use std::process::{Command, exit, id as getpid};
...
fn wait_for_status<P: Into<Option<Pid>>>(
    ...
    match Errno::result(res)? {
        0 => Ok(WaitResult { status: WaitStatus::StillAlive, status_raw: status, result: 0 }),
        res => Ok(WaitResult { status: WaitStatus::from_raw(Pid::from_raw(res), status)?, status_raw: status, result: res })
    }
}
...
impl Jail {
    ...
    fn load_plugins(&mut self) {
        for glob_path in ["/etc/backend.ai/jail/plugins/*.so", "/opt/jail/plugins/*.so", "./*.so"] {
                ...
                match self.load_plugin(&path) {
                    Ok((p, l)) => {
                        ...
                        for syscall in &hooked_syscalls {
                            if syscall.hook_type == HookType::PRE || syscall.hook_type == HookType::BOTH {
                                ...
                                self.pre_execution_hooks.insert(syscall.name.to_string(), hook);
                            }
                            if syscall.hook_type == HookType::POST || syscall.hook_type == HookType::BOTH {
                                let mut hook: Vec<String>;
                                ...
                                self.post_execution_hooks.insert(syscall.name.to_string(), hook);
                            }
                        }
                    ...
                    Err(e) => {
                        warn!("error while loading plugin {:?}: {}, skipping plugin import", path, e.to_string());
                    }
                }
            }
        }
    ...
    fn load_plugin(&self, path: &PathBuf) -> Result<(Box<dyn PluginInterface>, libloading::Library), libloading::Error> {
        let lib = unsafe {
        ...
        let constructor: libloading::Symbol<extern "Rust" fn() -> *mut dyn PluginInterface> = unsafe { lib.get(b"load") }?;
    ...
    fn trace_process(&mut self, child: Pid) {
        ...
        let ptrace_options = 
            PtraceOptions::PTRACE_O_TRACESECCOMP |
    ...
    fn handle_ptrace_event(&mut self, child: Pid, target: Pid, signal: Signal, evt_num: libc::c_int) -> Result<(), Errno> {
        ...
        match signal {
            Signal::SIGTRAP => {
                match evt_type {
                    ...
                    ptrace::Event::PTRACE_EVENT_SECCOMP => {
                        ...
                        match syscall_name.as_str() {
                            "fork" | "vfork" | "clone" | "clone3" => {
                                ...
                                let jail_path = panic_if_err!(env::current_exe()).display().to_string();
                            ...
                            "execve" | "execveat" => {
                                ...
                                allow = match target_exec_path {
                                    ...
                                    _ if self.policy_inst.check_path_op(&target_exec_path, PathOps::OpExec, 0) => true,
                                    _ => {
                        ...
                        match self.post_execution_hooks.get(&syscall_name) {
                            Some(hooks) => {
                                ...
                                for hook in hooks {
                                    let plugin = self.plugins.get(hook).unwrap();
                                    match plugin.post_execution_hook(&syscall_name, target, &regs) {
                                        1 => {},
                                        0 if allow => { allow = false },
                                        err => {
                                            let errno: Errno = unsafe { std::mem::transmute(err) };
                    ...
                    ptrace::Event::PTRACE_EVENT_CLONE | ptrace::Event::PTRACE_EVENT_FORK | ptrace::Event::PTRACE_EVENT_VFORK => {
    ...
    fn run_child(&mut self) {
        ...
        if !self.cli.noop {
            ...
            let mut filter = panic_if_err!(ScmpFilterContext::new_filter(ScmpAction::Errno(libc::EPERM)));
        ...
        #[cfg(target_arch = "aarch64")]
            ...
            for syscall in self.policy_inst.get_allowed_syscalls() {
                ...
                if plugin_hooked_syscalls.contains(&syscall) {
                    debug!("Skipping allowing syscall {} (this will be traced by plugin)", syscall);
                    continue;
                }
            ...
            for syscall in self.policy_inst.get_traced_syscalls() {
                match ScmpSyscall::from_name(&syscall) {
                    Ok(syscall_id) => panic_if_err!(filter.add_rule(ScmpAction::Trace(1), syscall_id)),
                    Err(e) => {
            ...
            for syscall in ["kill", "killpg", "tkill", "tgkill"] {
                match ScmpSyscall::from_name(&syscall) {
                    Ok(syscall_id) => panic_if_err!(filter.add_rule(ScmpAction::Trace(2), syscall_id)),
                    Err(e) => {
                        warn!("error while tracing syscall {}: {}", syscall, e);
                    }
                }
            ...
            for syscall in plugin_hooked_syscalls {
                match ScmpSyscall::from_name(&syscall) {
                    Ok(syscall_id) => panic_if_err!(filter.add_rule(ScmpAction::Trace(3), syscall_id)),
                    Err(e) => {
                        warn!("error while tracing syscall {}: {}", syscall, e);
                    }
                }
        ...
        let error = Command::new(&bin_path).args(self.cli.args.get_mut(1..).unwrap()).exec();
